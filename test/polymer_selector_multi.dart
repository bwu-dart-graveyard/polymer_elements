...
void oneMutation(dom.Element node, options, Function cb) {
  var o = new dom.MutationObserver((List<dom.MutationRecord> mutations, dom.MutationObserver observer) {
    cb();
    observer.disconnect();
  });
  ...
  initPolymer().run(() {
    Polymer.onReady.then((e) {
      test('polymer-selector-multi', () {
        ...
        // set selected
        //dom.Platform.flush();
        s.selected = [0, 2]; // TODO doesn't work as expected https://code.google.com/p/dart/issues/detail?id=17472
        s.dummy = 'dummy'; // TODO remove workaround for ^
        oneMutation(s, {'attributes': true}, () {
          new async.Future(() { // TODO workaround for https://code.google.com/p/dart/issues/detail?id=14496
            ...
            s.children[0].dispatchEvent(new dom.CustomEvent('click', canBubble: true)); // change to 'tap' when it's working
            // check selected
